...
namespace LASYS.Infrastructure.Persistence.Repositories
{
    public class WorkOrderRepository : IWorkOrderRepository
    {
        ...
        /// </summary>
        public async Task<bool> CheckExistsAsync(string sql, params MySqlParameter[] parameters)
        {
        ...
        /// </summary>
        public async Task<string> GetItemStringAsync(string sql, params MySqlParameter[] parameters)
        {
            using var conn = await _factory.CreateConnectionAsync();
        ...
        /// </summary>
        public async Task<DataTable> GetListAsync(string sql, params MySqlParameter[] parameters)
        {
            using var conn = await _factory.CreateConnectionAsync();
        ...
        /// <summary>
        /// Executes the query and returns a sequence of T.
        /// Fixed: made this a generic method, awaited QueryAsync and returned IEnumerable&lt;T&gt;.
        /// </summary>
        public async Task<IEnumerable<T>> GetListAsync<T>(string sql, params MySqlParameter[] parameters)
        {
            using var conn = await _factory.CreateConnectionAsync();

            // Await the QueryAsync result (returns IEnumerable<T>), don't use 'using' on the Task.
            var data = await conn.QueryAsync<T>(sql, BuildParameters(parameters));
        ...
        /// </summary>
        public async Task<bool> CheckInstructionBarcodeExistsAsync(string barcode)
        {
            var p = new MySqlParameter("@barcode", barcode);
            return await CheckExistsAsync(CheckInsBarcodeExistsSql, p);
        }
        ...
        public async Task<string> GetItemCodeByBarcodeAsync(string barcode)
        {
            var p = new MySqlParameter("@barcode", barcode);
            return await GetItemStringAsync(GetItemCodeSql, p);
        }
        ...
        public async Task<bool> CheckIsScrappedAsync(string statusColumn, string itemCode, string lotNo)
        {
            // statusColumn comes from a fixed internal constant – no user data
            var sql = string.Format(CheckScrappedSql, statusColumn);
            return await CheckExistsAsync(sql,
        ...
        public async Task<bool> CheckIsPairedAsync(string itemCode, string lotNo)
        {
            return await CheckExistsAsync(CheckPairedSql,
                new MySqlParameter("@itemCode", itemCode));
        }
        ...
        public async Task<string> GetUpdatedUbBarcodeAsync(string itemCode, string lotNo)
            => await GetItemStringAsync(GetUpdatedUbBarcodeSql,
                   new MySqlParameter("@itemCode", itemCode),
                   new MySqlParameter("@lotNo", lotNo));
        ...
        public async Task<int> GetLatestRevisionAsync(string itemCode, string lotNo)
        {
            var value = await GetItemStringAsync(GetLatestRevisionSql,
                new MySqlParameter("@itemCode", itemCode),
                new MySqlParameter("@lotNo", lotNo));
        ...
        public async Task<IEnumerable<ProductMaster>> GetProductDataAsync(string itemCode, int revision)
        {
            var history = await GetListAsync<ProductMaster>(GetProductHistorySql,
                new MySqlParameter("@itemCode", itemCode),
            ...
            return await GetListAsync<ProductMaster>(GetProductMasterSql,
                new MySqlParameter("@itemCode", itemCode));
        }
        ...
        public async Task<InstructionPrintSnapshot?> GetInstructionPrintSnapshotAsync(string instructionColumn,
                                                     string targetColumn,
                                                     string printTable,
                                                     string itemCode,
                                                     string lotNo,
                                                     string instructionCode)
        {
            ...
            var result = await GetListAsync<InstructionPrintSnapshot>(sql,
                new MySqlParameter("@itemCode", itemCode),
                new MySqlParameter("@lotNo", lotNo),
        ...
        public async Task<long> GetMaxSequenceNumberAsync(string printTable, string itemCode, string lotNo)
        {
            string sql = $@"
                SELECT IFNULL(MAX(sequence_number), 0)
                FROM   {printTable}
            ...
            string value = await GetItemStringAsync(sql,
                new MySqlParameter("@itemCode", itemCode),
                new MySqlParameter("@lotNo", lotNo));
        ...
        /// </summary>
        public async Task<bool> CheckPrintRecordExistsAsync(string tableName, string itemCode, string lotNo)
        {
        ...
        /// </summary>
        public async Task<int> SavePrintRecordAsync(string printTable, DataRow row,
