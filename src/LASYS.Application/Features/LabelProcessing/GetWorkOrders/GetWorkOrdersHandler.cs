    ...
    public class GetWorkOrdersHandler : IRequestHandler<GetWorkOrdersQuery, Result<PaginatedList<GetWorkOrdersResult>>>
    {
        ...
        public async Task<Result<PaginatedList<GetWorkOrdersResult>>> Handle(GetWorkOrdersQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = (await connection.QueryAsync<GetWorkOrdersResult>(query, parameters)).ToList();
                var totalCount = result.Count > 0 ? result[0].TotalCount : await connection.ExecuteScalarAsync<int>($"SELECT COUNT(*) {baseSql}", parameters);
                return Result.Success(new PaginatedList<GetWorkOrdersResult>(result, totalCount, pageNo, pageSize));
            }
            catch (Exception ex)
            {
